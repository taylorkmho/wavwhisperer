...
import { withAnthropicErrorHandler } from "./client";
...
Someone should be able to read it and decide where and when to surf.`;
  const tone =
    "The tone should be funny and engaging. Avoid referencing Hawaiian cultural elements in jest.";
...
Then small on the south, come what may!`,
  ...
  return withAnthropicErrorHandler(async () => {
    ...
    return { poem: lines, model: response.model };
  });
}
